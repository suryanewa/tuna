import * as React from "react";

export interface BooleanSmallProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
}

export const BooleanSmall = React.forwardRef<SVGSVGElement, BooleanSmallProps>(
  ({ size = 24, className, ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      {...props}
    >
      <path fillRule="evenodd" clipRule="evenodd" d="M10 8H14C16.2091 8 18 9.79086 18 12C18 14.2091 16.2091 16 14 16H10C7.79086 16 6 14.2091 6 12C6 9.79086 7.79086 8 10 8ZM5 12C5 9.23857 7.23857 7 10 7H14C16.7614 7 19 9.23857 19 12C19 14.7614 16.7614 17 14 17H10C7.23857 17 5 14.7614 5 12ZM10 14C8.89543 14 8 13.1046 8 12C8 10.8954 8.89543 10 10 10C11.1046 10 12 10.8954 12 12C12 13.1046 11.1046 14 10 14ZM7 12C7 10.3431 8.34314 9 10 9C11.6569 9 13 10.3431 13 12C13 13.6569 11.6569 15 10 15C8.34314 15 7 13.6569 7 12Z" fill="currentColor" fillOpacity={0.9}/>
    </svg>
  )
);

BooleanSmall.displayName = "BooleanSmall";
